...
# ------------------------ functions -------------------------------------------
function bootstrap() {
  cargo run --profile release -j 12 -p aleph-node --features liminal-runtime-benchmarks -- bootstrap-chain \
    --base-path /tmp/ \
    --account-ids $NODE_ID \
...
function benchmark() {
  ...
  cargo run --profile release -j 12 -p aleph-node --features liminal-runtime-benchmarks -- benchmark pallet \
