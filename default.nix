...
, makeOverridable ? (import <nixpkgs> {}).lib.makeOverridable
}:
let
  defaultArgs =
    {
  ...
  derivation = args:
    let
      providedCargoHome = args.cargoHomePath != "";
      cargoHome = builtins.path { path = builtins.toPath args.cargoHomePath; name = "cargo-home"; };

      nixpkgs = args.versions.nixpkgs;
      ...
      llvm = args.versions.llvm;
      env = if args.keepDebugInfo then nixpkgs.keepDebugInfo args.versions.stdenv else args.versions.stdenv;

      # tool for conveniently building rust projects
      naersk = args.versions.naersk.override { stdenv = env; };
      ...
      customRocksdb = args.versions.customRocksDB;
      ...
      prefixedFeatureList = builtins.concatLists (nixpkgs.lib.mapAttrsToList featuresIntoPrefixedFeatures args.crates);
      ...
      inherit (args.versions.gitignore) gitignoreFilter;
      ...
      customBuild = old:
        if args.customBuildCommand != "" then args.customBuildCommand else old;
    in
    with nixpkgs; naersk.buildPackage rec {
      ...
      nativeBuildInputs = [
        ...
        llvm.libclang
        protobuf
      ] ++ nixpkgs.lib.optional args.setInterpreter.substitute patchelf;
      propagatedBuildInputs = nixpkgs.lib.optional args.useCustomRocksDb customRocksdb;
      ...
      shellHook = ''
        ...
        export RUSTFLAGS="${args.rustflags}"
      ...
      postInstall = ''
        ...
        ${nixpkgs.lib.optionalString args.setInterpreter.substitute "if [[ -d $out/bin && $(find $out/bin/ -type f | wc -l) > 0 ]]; then find $out/bin -type f | xargs patchelf --set-interpreter ${args.setInterpreter.path}; fi"}
      '';

    };
in
makeOverridable derivation defaultArgs
