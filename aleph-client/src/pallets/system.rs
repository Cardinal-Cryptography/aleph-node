...
pub trait SystemApi {
    async fn get_free_balance(&self, account: AccountId, at: Option<H256>) -> Balance;
}

#[async_trait::async_trait]
pub trait SystemSudoApi {
    async fn set_code(&self, code: Vec<u8>, status: TxStatus) -> anyhow::Result<H256>;
    async fn fill_block(&self, target_ratio_percent: u8, status: TxStatus) -> anyhow::Result<H256>;
}
