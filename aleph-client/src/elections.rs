...
pub fn get_committee_size<C: AnyConnection>(connection: &C, block_hash: Option<H256>) -> u32 {
    connection
        ...
        .unwrap_or_else(|| panic!("Failed to obtain CommitteeSize for block hash: {:?}.", block_hash))
}

pub fn get_validator_block_count<C: AnyConnection>(connection: &C, account_id: &AccountId, block_hash: Option<H256>)-> Option<u32> {
    connection
        .as_connection()
        .get_storage_map(
            "Elections",
