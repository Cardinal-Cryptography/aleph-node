...
./target/release/aleph-node bootstrap-chain --millisecs-per-block 2000 --session-period 10 --sessions-per-era 3 --base-path "$BASE_PATH" --account-ids "$validator_ids_string" --chain-type local > "$BASE_PATH/chainspec.json"
...
for i in $(seq 0 "$(( N_VALIDATORS + N_NON_VALIDATORS - 1 ))"); do
  ...
  ./target/release/aleph-node \
    ...
    -lafa=debug \
    -laleph-party=debug \
    -laleph-network=debug --rpc-cors="all" \
    "$@" \
    2> $auth.log > /dev/null & \
done
