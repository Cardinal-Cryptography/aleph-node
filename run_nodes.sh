...
./target/release/aleph-node bootstrap-chain --millisecs-per-block 1000 --session-period 40 --sessions-per-era 3 --base-path "$BASE_PATH" --account-ids "$validator_ids_string" --chain-type local > "$BASE_PATH/chainspec.json"
