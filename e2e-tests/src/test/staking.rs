...
use aleph_client::{
    ...
    staking_validate, wait_for_full_era_completion, wait_for_session, KeyPair, RootConnection,
    SignedConnection,
};
use primitives::{
...
pub fn staking_era_payouts(config: &Config) -> anyhow::Result<()> {
    ...
    validator_accounts.par_iter().for_each(|account| {
        let connection = SignedConnection::new(create_connection(node), account.clone());
    ...
    stashes_accounts_key_pairs
        ...
        .for_each(|(nominator, nominee)| {
            let connection = SignedConnection::new(create_connection(node), nominator.clone());
    ...
    validator_accounts.into_par_iter().for_each(|key_pair| {
        let stash_connection = SignedConnection::new(create_connection(node), key_pair.clone());
...
pub fn staking_new_validator(config: &Config) -> anyhow::Result<()> {
    ...
    let connection: RootConnection = SignedConnection::new(create_connection(node), sender).into();
    ...
    let stash_connection = SignedConnection::new(create_connection(node), stash.clone());
    ...
    let controller_connection = SignedConnection::new(create_connection(node), controller.clone());
