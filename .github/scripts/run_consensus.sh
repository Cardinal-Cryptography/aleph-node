...
function generate_authorities {
  ...
  for node_index in $(seq 0 $((authorities_count - 1))); do
    ...
    account_ids+=($(docker run --rm -v $(pwd)/docker/data:/data --entrypoint "/bin/sh" aleph-node:latest -c "aleph-node key inspect //$node_index | grep \"SS58 Address:\" | awk \"{print \\\$3;}\""))
  done
  echo "${account_ids[*]}"
}
...
function generate_chainspec {
  ...
  docker run --rm -v $(pwd)/docker/data:/data --entrypoint "/bin/sh" -e RUST_LOG=debug aleph-node:latest -c \
...
function generete_bootnode_peer_id {
  ...
  export BOOTNODE_PEER_ID=$(docker run --rm -v $(pwd)/docker/data:/data --entrypoint "/bin/sh" -e RUST_LOG=info aleph-node:latest -c "aleph-node key inspect-node-key --file /data/$bootnode_account/p2p_secret")
}
