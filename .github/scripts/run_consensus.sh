...
docker run -v $(pwd)/docker/data:/data --entrypoint "/bin/sh" -e DAMIAN -e TOMASZ -e ZBYSZKO -e HANSU -e JULIA -e MARCIN -e RUST_LOG=info \
 aleph-node:latest -c "aleph-node bootstrap-chain --base-path /data --chain-id a0dnet1-e2e --account-ids $DAMIAN,$TOMASZ,$ZBYSZKO,$HANSU,$JULIA,$MARCIN --sudo-account-id $DAMIAN > /data/chainspec.json"
