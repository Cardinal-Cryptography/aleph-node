...
// Plan:
//
// - Generate 1024 accounts
// - set validators status to Validate
// - set them to nominate
// - wait a full era
// - send payout stakers xt
//
//
//
//
fn main() -> Result<(), anyhow::Error> {
    ...
    let accounts = (..1024).map(derive_user_account);
    batch_endow_account_balances(&connection, accounts, 1);

    // 2. Set validators status to Validate
    let validator_accounts = (..4).map(derive_user_account);
    validator_accounts
        .par_iter()
    ...
    accounts
        .par_iter()
        .for_each(|nominator| bond(address, NOMINATOR_STAKE, &nominator, &nominator));
...
fn batch_endow_account_balances(
    connection: &Connection,
    account_keys: impl Iterator<Item = Pair>,
    endowment: u128,
) {
