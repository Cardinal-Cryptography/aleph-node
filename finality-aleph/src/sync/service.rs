...
use crate::{
    ...
    sync::{
        data::{NetworkData, State, VersionWrapper, VersionedNetworkData},
        forest::Interest,
    ...
    > Service<J, N, CE, CS, V, F>
{
    ...
    fn send_request_for(&mut self, block_id: BlockIdFor<J>, peers: HashSet<N::PeerId>) {
        let state = match self.handler.state() {
            Ok(state) => state,
            Err(e) => {
                warn!(
        ...
        let data = NetworkData::Request(block_id, state);
    ...
    fn handle_request(&mut self, requested_id: BlockIdFor<J>, state: State<J>, peer: N::PeerId) {
        match self.handler.handle_request(requested_id, state) {
            Ok(action) => self.perform_sync_action(action, peer),
            Err(e) => {
                warn!(
                    target: LOG_TARGET,
    ...
    fn handle_network_data(&mut self, data: NetworkData<J>, peer: N::PeerId) {
        ...
        match data {
            ...
            Justifications(justifications, state) => {
                self.handle_justifications(justifications, peer.clone());
                self.handle_state(state, peer)
            }
            Request(requested_id, state) => {
                self.handle_request(requested_id, state.clone(), peer.clone());
                self.handle_state(state, peer)
            }
        }
    }
    ...
    fn handle_task(&mut self, block_id: BlockIdFor<J>) {
        ...
        match self.handler.block_state(&block_id) {
            TopRequired(know_most) => {
                self.send_request_for(block_id.clone(), know_most);
