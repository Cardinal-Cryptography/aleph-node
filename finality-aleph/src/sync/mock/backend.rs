...
impl Backend {
    ...
    pub fn import(&self, header: MockHeader) {
        ...
        let parent_id = match header.parent_id() {
            Some(id) => id,
            None => panic!("importing block without a parent: {self:?}"),
        };

        if storage.blockchain.contains_key(&header.id()) {
            panic!("importing an already imported block: {self:?}")
        }

        if !storage.blockchain.contains_key(&parent_id) {
            panic!("importing block without an imported parent: {self:?}")
        }

        if header.id().number() != parent_id.number() + 1 {
            panic!("importing block without a correct parent: {self:?}")
        }
