...
type CallArgs = (THash, TBlockNumber);
...
impl RequestBlocks<TBlock> for MockedBlockRequester {
    fn request_justification(&self, hash: &THash, number: TBlockNumber) {
        self.mock.invoke_with((*hash, number))
    }

    fn request_stale_block(&self, _hash: THash, _number: TBlockNumber) {
