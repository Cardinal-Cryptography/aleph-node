    ...
    pub async fn run(mut self, mut exit: oneshot::Receiver<()>) {
        ...
        loop {
            tokio::select! {
                ...
                Some((peer_id, maybe_data_for_network)) = worker_results.next() => {
                    use AddResult::*;
                    match maybe_data_for_network {
                        Some(data_for_network) => match self.manager.add_connection(peer_id.clone(), data_for_network) {
                        ...
                        None => if let Some(addresses) = self.manager.peer_addresses(&peer_id) {
                            self.spawn_new_outgoing(peer_id, addresses, result_for_parent.clone());
                        }
                    }
                },
